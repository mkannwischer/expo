...
otcrypto_status_t otcrypto_mlkem512_keygen(otcrypto_byte_buf_t public_key,
  ...
  HARDENED_TRY(entropy_csrng_generate(&kEntropyEmptySeed, (uint32_t *)randomness,
                                      sizeof(randomness) / 4,
                                      /*fips_check=*/kHardenedBoolTrue));
  HARDENED_TRY(entropy_csrng_uninstantiate());

  int result = mlkem512_keypair_derand(public_key.data, secret_key.data, randomness);
  if (result != 0) {
    return OTCRYPTO_FATAL_ERR;
  }
...
otcrypto_status_t otcrypto_mlkem768_keygen(otcrypto_byte_buf_t public_key,
  ...
  int result = mlkem768_keypair_derand(public_key.data, secret_key.data, randomness);
  if (result != 0) {
    return OTCRYPTO_FATAL_ERR;
  }
...
otcrypto_status_t otcrypto_mlkem1024_keygen(otcrypto_byte_buf_t public_key,
  ...
  int result = mlkem1024_keypair_derand(public_key.data, secret_key.data, randomness);
  if (result != 0) {
    return OTCRYPTO_FATAL_ERR;
  }
