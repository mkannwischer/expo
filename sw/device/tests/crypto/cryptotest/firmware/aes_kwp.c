  ...
  switch (uj_op) {
    ...
    case kCryptotestAesKwpOperationUnwrap: {
      ...
      if (success == kHardenedBoolTrue) {
        uj_output.success = true;
        // The actual plaintext length is encoded in the KWP header
        // and verified by aes_kwp_unwrap. The output buffer is the
        // same size as the ciphertext minus the 8-byte header.
