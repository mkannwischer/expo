...
static void test_mlkem512_randomized(void){
  ...
  otcrypto_byte_buf_t key_b_buf = {.data = key_b, .len = sizeof(key_b)};
  t0 = profile_start();
  CHECK_STATUS_OK(otcrypto_mlkem512_encapsulate(pk_const_buf, ct_buf, key_b_buf));
...
static void test_mlkem768_randomized(void){
  ...
  otcrypto_byte_buf_t key_b_buf = {.data = key_b, .len = sizeof(key_b)};
  t0 = profile_start();
  CHECK_STATUS_OK(otcrypto_mlkem768_encapsulate(pk_const_buf, ct_buf, key_b_buf));
...
static void test_mlkem1024_randomized(void){
  ...
  otcrypto_byte_buf_t key_b_buf = {.data = key_b, .len = sizeof(key_b)};
  t0 = profile_start();
  CHECK_STATUS_OK(otcrypto_mlkem1024_encapsulate(pk_const_buf, ct_buf, key_b_buf));
